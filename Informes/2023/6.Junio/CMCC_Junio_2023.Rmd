...
MP10_Modelados <- list.files(path = "CMCC/Modelados/V1/2023/6.junio/MP10",
                               pattern = "*.xlsx",
                               full.names = TRUE) %>%
  lapply(read_excel)
...
CMCC_Mayo_Obs <- read_excel("CMCC/Observados/2023/6.junio/Informe.xlsx")

names(CMCC_Mayo_Obs) <- trimws(names(CMCC_Mayo_Obs))
...
CMCC_Mayo_Obs$MP_10 <- abs(CMCC_Mayo_Obs$MP_10)
...
Met_Modelados <- list.files(path = "CMCC/Modelados/V1/2023/6.junio/Meteorologia",    
                            pattern = "*.xlsx",
                            full.names = TRUE) %>% 
  lapply(read_excel) 
...
df <- data.frame(`Nº Proyecto` = c("G12-21"),
                 Elaboración = c("CSD 07-07-23"),
                 `Revisión A` = c("MTV 07-07-23"),
                 Aprobó = c("PSH 07-07-23"),
                 Versión = "V1")
...
Para evaluar el desempeño del modelo de pronóstico, se dispone de la base de datos `r MesDelReporte` del 2023, de las concentraciones de MP10 medidas en la estación Campamento, la cual se compara con las concentraciones de MP10 pronosticadas por **GEOMET** para igual período.
...
Velocidad <- Velocidad[1:450,]

Velocidad <- pivot_longer(Velocidad, cols = c("VEL","VEL_MOD"))
...
MP10 <- pivot_longer(MP10, cols = c("MP_10", "MP_10_MOD"))
...
El análisis estadístico de las concentraciones de MP10 observados y medidos en la estación Campamento se muestra en la tabla III.4. De ella se observa que el modelo presenta un buen desempeño debido a que los estadígrafos se encuentran dentro de los rangos aceptables.
...
ExactitudMP10 <- FechayValor_MP10Obs %>% 
  mutate(MP_10_MOD = FechayValor_MP10Mod$VALOR,
         Bueno = case_when((between(MP_10, 0, 130) & between(MP_10_MOD, 0, 130)) | (!between(MP_10, 0, 130) & (!between(MP_10_MOD, 0, 130))) ~ 1,
                           TRUE ~ 0),
         Alerta1 = case_when((between(MP_10, 130, 180) & between(MP_10_MOD, 130, 180)) | (!between(MP_10, 130, 180) & (!between(MP_10_MOD, 130, 180))) ~ 1,
                             TRUE ~ 0),
         Alerta2 = case_when((MP_10 > 180 & MP_10_MOD > 180) | (MP_10 < 180 & MP_10_MOD < 180) ~ 1,
                             TRUE ~ 0))
